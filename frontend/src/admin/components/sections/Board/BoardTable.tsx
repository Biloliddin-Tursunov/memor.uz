          ...
          <div className="flex flex-col bg-transparent min-h-[400px]">
            {filteredTasks.length > 0 ? (
              filteredTasks.map((task) => (
                <TableRow key={task.id} task={task} onUpdate={onUpdateTask} onOpenModal={() => onRowClick(task)} readOnly={!canEdit(currentUser.role)} t={t} />
              ))
            ) : (
              <div className="flex flex-col items-center justify-center py-40 text-textMuted italic opacity-20 scale-150">
                <SmartIcon name="Database" size={100} glow={false} />
                <span className="mt-4 font-black uppercase tracking-widest text-xs">{t('no_data')}</span>
              </div>
            )}
          </div>
        </div>
