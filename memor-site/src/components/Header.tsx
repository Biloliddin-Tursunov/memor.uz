...
const Header: React.FC<HeaderProps> = ({ activeRoute, onNavigate, onSearchOpen, language, theme, setTheme }) => {
  ...
  const headerClass = `fixed top-0 left-0 w-full z-40 transition-all duration-300 border-b py-4 bg-parchment/80 dark:bg-slate-900/80 backdrop-blur-xl shadow-sm border-graphite/5 dark:border-white/5`;
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      handleNavigate(`${PageRoute.SEARCH}?q=${encodeURIComponent(searchQuery)}` as any);
      setSearchQuery('');
    }
  };
    ...
    <nav className={className}>
      {NAV_ITEMS.map((item) => {
        ...
        return (
          ...
          >
            <span className={`font-display text-[10px] sm:text-xs lg:text-lg tracking-[0.2em] md:tracking-[0.25em] font-bold uppercase transition-all duration-500 ${isActive ? 'text-teal' : `${textColor} group-hover/btn:text-sepia`
              }`}>
              {label}
            </span>
            <span className={`h-[1.5px] bg-sepia mt-1 transition-all duration-500 ${isActive ? 'w-full' : 'w-0 group-hover/btn:w-full opacity-60'
              }`}></span>
          </button>
        );
      })}
  ...
  return (
    <>
          ...
          <div className="flex md:hidden flex-col">
            <div className="flex items-center justify-between w-full">
              ...
              <div className="flex items-center gap-2">
                <LanguageSwitcher currentLang={language} className='mr-2' />
      ...
      <div
        className={`fixed top-0 right-0 h-full w-full sm:w-[480px] bg-parchment/95 dark:bg-slate-900/95 backdrop-blur-2xl z-50 shadow-2xl transform transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] border-l border-graphite/10 dark:border-white/5 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        <div className="h-full flex flex-col p-8 md:p-12 overflow-y-auto no-scrollbar">
          ...
          {/* Settings - Language removed, Search Input added */}
          <div className="space-y-8 mb-12">
            ...
            <div>
              <form onSubmit={handleSearchSubmit} className="relative group">
                <input
                  ...
                  className="w-full bg-transparent border-b border-graphite/20 dark:border-white/20 py-3 pr-8 text-xl md:text-2xl font-display text-graphite dark:text-white placeholder:text-graphite/30 dark:placeholder:text-white/30 focus:outline-none focus:border-sepia transition-all duration-300"
                />
                <button type="submit" className="absolute right-0 top-1/2 -translate-y-1/2 text-graphite/40 dark:text-white/40 group-hover:text-sepia transition-colors duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="flex flex-col gap-4">
                <label className="block text-[10px] font-bold uppercase tracking-[0.3em] text-graphite/30 dark:text-white/20">{t.theme}</label>
                <div className="flex bg-graphite/5 dark:bg-white/5 p-1 rounded-lg">
                  <button
                    onClick={() => setTheme('light')}
                    className={`flex-1 h-10 text-[10px] flex items-center justify-center gap-2 tracking-widest uppercase rounded-md transition-all duration-300 ${theme === 'light'
                      ? 'bg-white shadow-sm text-teal font-bold'
                  ...
                  <button
                    ...
                    className={`flex-1 h-10 text-[10px] flex items-center justify-center gap-2 tracking-widest uppercase rounded-md transition-all duration-300 ${theme === 'dark'
                      ? 'bg-slate-800 shadow-sm text-white font-bold'
                      : 'text-graphite/40 dark:text-white/20 hover:text-graphite/60 dark:hover:text-white/40'
                      }`}
                  >
          ...
          <div className="flex-grow space-y-4">
            {[
              ...
              <button
                key={item.route}
                onClick={() => handleNavigate(item.route)}
                className={`group flex items-center justify-between w-full text-left py-4 border-b border-graphite/5 dark:border-white/5 transition-all duration-300 ${item.isSearch ? 'text-teal dark:text-teal-400 font-medium' : 'text-graphite/60 dark:text-white/40 hover:text-sepia dark:hover:text-sepia'
                  }`}
              >
                <span className={`font-display text-3xl md:text-4xl tracking-tight transition-transform duration-300 group-hover:translate-x-2 ${item.isSearch ? 'font-serif italic' : ''
                  }`}>
                  {item.label}
                </span>
                {item.isSearch && (
                  <svg className="w-6 h-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                  </svg>
                )}
              </button>
