...
import { findNews } from '../../constants';
...
export const NewsDetail: React.FC<{ language: Language }> = ({ language }) => {
  ...
  if (!news) return <div className="p-20 text-center">Yangilik topilmadi.</div>;

  return (
    <div className="max-w-4xl mx-auto px-6 py-12 animate-in fade-in duration-700">
      <button 
        onClick={() => navigate(-1)} 
        className="mb-8 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-graphite/40 hover:text-teal transition-all"
      >
        &larr; {language === 'uz' ? 'Yangiliklarga qaytish' : 'Back'}
      </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4">
           <div className="sticky top-32 bg-white dark:bg-white/5 p-8 border border-graphite/5 shadow-sm rounded-sm">
              <h3 className="font-display text-lg mb-6 dark:text-white uppercase tracking-widest">Ulashish</h3>
