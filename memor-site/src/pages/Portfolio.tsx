...
const AlbumItem: React.FC<{ item: PortfolioItem; rotation: string }> = ({ item, rotation }) => {
  return (
    <div className={`relative group ${rotation} hover:rotate-0 transition-transform duration-500`}>
      ...
      <div className="absolute -top-1 -left-1 w-8 h-8 border-t-2 border-l-2 border-graphite/20 z-10"></div>
      <div className="absolute -top-1 -right-1 w-8 h-8 border-t-2 border-r-2 border-graphite/20 z-10"></div>
      <div className="absolute -bottom-1 -left-1 w-8 h-8 border-b-2 border-l-2 border-graphite/20 z-10"></div>
      <div className="absolute -bottom-1 -right-1 w-8 h-8 border-b-2 border-r-2 border-graphite/20 z-10"></div>
      ...
      <div className="bg-white p-2 shadow-sm group-hover:shadow-xl transition-shadow duration-300">
         <img 
            src={item.imageUrl} 
            alt={item.title} 
            className="w-full h-64 object-cover filter sepia-[0.3] contrast-[0.9]"
        />
      </div>
      ...
      <div className="mt-4 text-center">
        ...
        <p className="font-serif text-sm italic text-graphite/60 mt-1">
