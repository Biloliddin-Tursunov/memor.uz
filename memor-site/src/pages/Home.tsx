   ...
   return (
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12">
         ...
         <section className="mb-12 md:mb-20 relative group">
            <div className="relative overflow-hidden border border-graphite/10 dark:border-white/10 bg-white dark:bg-white/5 p-2 shadow-sm min-h-[60vh] md:min-h-[500px]">
               {/* Slide Content */}
               <div className="relative w-full h-[60vh] md:h-[500px]">
                  <img
                     src={carouselItems[currentSlide].image}
                     alt={carouselItems[currentSlide].title}
                     className="w-full h-full object-cover contrast-[1.1] transition-all duration-700"
                  />
                  ...
                  <div className="absolute bottom-0 left-0 p-6 md:p-12 w-full md:w-3/4 text-white">
                     ...
                     <h2
                        ...
                        onClick={() => {
                           // Carousel text click goes to Detail now if needed, or route
                           onNavigate(carouselItems[currentSlide].route);
                        }}
                     >
                     ...
                     <button
                        onClick={() => onItemClick({
                           id: carouselItems[currentSlide].id,
                           title: carouselItems[currentSlide].title,
                           description: carouselItems[currentSlide].desc,
                           imageUrl: carouselItems[currentSlide].image,
                           type: carouselItems[currentSlide].label
                        })}
                        className="mt-4 text-xs uppercase tracking-widest text-teal hover:text-white transition-colors"
                     >
         ...
         <section className="mb-16 md:mb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               {MOCK_ARTICLES.slice(0, 2).map((article) => (
                  <div
                     key={article.id}
                     className="group cursor-pointer bg-white dark:bg-white/5 border border-graphite/10 dark:border-white/10 p-4 transition-transform hover:-translate-y-1"
                     onClick={() => onItemClick({
                        id: article.id,
                        title: article.title,
                  ...
                  >
                     ...
                     <p className="font-serif text-sm text-graphite/70 dark:text-gray-400 line-clamp-3">{article.excerpt}</p>
                  </div>
               ))}
               <div
                  className="group cursor-pointer bg-white dark:bg-white/5 border border-graphite/10 dark:border-white/10 p-4 transition-transform hover:-translate-y-1"
                  onClick={() => onItemClick({
                     ...
                     subtitle: MOCK_VIDEOS[0].author,
                     description: "Video darslikni tomosha qiling.",
                     imageUrl: MOCK_VIDEOS[0].thumbnailUrl,
                     type: MOCK_VIDEOS[0].type
                  })}
               >
                  <div className="overflow-hidden mb-4 h-48 relative">
                     <img src={MOCK_VIDEOS[0].thumbnailUrl} alt={MOCK_VIDEOS[0].title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                  ...
                  <p className="font-serif text-sm text-graphite/70 dark:text-gray-400">Video darslik • {MOCK_VIDEOS[0].duration}</p>
               </div>
            </div>
         </section>
         ...
         <section className="mb-16 md:mb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
               {/* Event Highlight */}
               <div className="bg-white dark:bg-white/5 border border-graphite/10 dark:border-white/10 p-6 md:p-8 shadow-sm relative mt-4 md:mt-0">
                  ...
                  <div className="flex flex-col md:flex-row gap-4 md:gap-6 text-xs md:text-sm font-mono text-graphite/60 dark:text-gray-400 mb-6">
                  ...
                  <p className="font-serif text-base md:text-lg text-graphite/80 dark:text-gray-300 mb-6">{MOCK_EVENTS[0].description}</p>
                  <button
                     onClick={() => onItemClick({
                        id: MOCK_EVENTS[0].id,
                        title: MOCK_EVENTS[0].title,
                        subtitle: `${MOCK_EVENTS[0].date} | ${MOCK_EVENTS[0].location}`,
                        description: MOCK_EVENTS[0].description,
                        type: 'Tadbir'
                     })}
                     className="w-full py-3 border border-graphite/20 hover:bg-graphite hover:text-white transition-colors uppercase text-xs tracking-widest dark:border-white/20 dark:hover:bg-teal"
                  >
                     Batafsil ma'lumot
                  </button>
               </div>
               ...
               <div
                  ...
                  onClick={() => onItemClick({
                     ...
                     title: MOCK_PROJECTS[0].title,
                     subtitle: `${MOCK_PROJECTS[0].status} | ${MOCK_PROJECTS[0].location}`,
                     description: MOCK_PROJECTS[0].description,
                     imageUrl: MOCK_PROJECTS[0].imageUrl,
                     type: 'Loyiha'
                  })}
               >
                  <div className="absolute inset-0 bg-graphite/10 dark:bg-white/10 transform rotate-2 transition-transform group-hover:rotate-1"></div>
                  <div className="relative bg-white dark:bg-white/5 border border-graphite/10 dark:border-white/10 h-full p-1">
                     <div className="relative h-64 md:h-full overflow-hidden">
                        <img src={MOCK_PROJECTS[0].imageUrl} alt={MOCK_PROJECTS[0].title} className="w-full h-full object-cover" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/90 to-transparent flex flex-col justify-end p-6 md:p-8 text-white">
         ...
         <section className="mb-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
               {MOCK_CREATIONS.map((item, idx) => (
                  ...
                  >
                     <img
                        src={item.imageUrl}
                        alt={item.title}
                        className="w-full h-full object-cover transition-all duration-700 transform group-hover:scale-105 min-h-[150px]"
                     />
               ...
               <div
                  className="relative group cursor-pointer overflow-hidden bg-graphite/5 dark:bg-white/10 flex items-center justify-center min-h-[150px]"
                  onClick={() => onNavigate(PageRoute.CREATION)}
               >
