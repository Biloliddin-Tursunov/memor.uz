    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="text-center mb-8 border-b-2 border-ink/10 pb-6">
          ...
          <h2 className="font-serif text-3xl text-ink font-bold">Confidential</h2>
          <p className="font-typewriter text-sm text-ink/60 mt-2">Authorized Personnel Only</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block font-typewriter text-xs uppercase tracking-widest text-ink/50 mb-1">Identity</label>
            <input
              type="email"
              value={email}
              ...
              placeholder="admin@memor.com"
              disabled={isLoading}
            />
          </div>

          <div>
            <label className="block font-typewriter text-xs uppercase tracking-widest text-ink/50 mb-1">Access Code</label>
            <input
              type="password"
              value={password}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-ink text-white font-serif text-lg py-3 hover:bg-ink-light transition-colors shadow-lg mt-4 flex items-center justify-center gap-2 disabled:opacity-70"
          >
            {isLoading ? (
              <>
