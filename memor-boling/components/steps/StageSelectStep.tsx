...
import { MapPin, Home, Lightbulb, LucideIcon, ArrowRight } from 'lucide-react';
...
const Option: React.FC<OptionProps> = ({ icon: Icon, title, desc, onClick }) => (
  <button 
    onClick={onClick}
  ...
  >
    ...
    <p className="text-slate-500 leading-relaxed text-xs md:text-sm font-medium">{desc}</p>
...
const StageSelectStep: React.FC<{ onNext: (type: string) => void }> = ({ onNext }) => {
  return (
    <div className="h-full flex flex-col pt-4">
      <h2 className="text-2xl md:text-3xl font-serif mb-8 text-slate-900 text-center">Qaysi bosqichdamiz?</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full max-w-5xl mx-auto">
        ...
        <Option 
          icon={Home}
          title="Uy bor" 
          desc="Mavjud uyni qayta ta'mirlash yoki dizayn qilish."
