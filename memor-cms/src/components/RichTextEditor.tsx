...
import {
    ...
    Type as TypeIcon, X, Plus, Minus, Highlighter, Palette, File
} from 'lucide-react';
...
export const RichTextEditor: React.FC<RichTextEditorProps> = ({ content, onChange, placeholder, onRequestMedia, onImageUpload }) => {
        ...
        <div className="flex flex-col w-full bg-white text-[#37352F] border border-[#e0e0e0] rounded shadow-sm relative font-sans">
            ...
            <div className="bg-white border-b border-[#f1f1ef] p-1.5 flex flex-wrap gap-0.5 items-center z-30 sticky top-0 shadow-sm overflow-x-auto no-scrollbar">
                ...
                <div className="relative">
                    <button onClick={() => setShowFontSizeMenu(!showFontSizeMenu)} className="flex items-center gap-1 px-2 py-1.5 hover:bg-gray-100 rounded text-[11px] font-bold transition-all">
                        <TypeIcon size={16} /> <ChevronDown size={10} />
                    </button>
                    {showFontSizeMenu && (
                ...
                <ToolbarBtn icon={<ImageIcon size={16} />} onClick={handleImageUpload} />
            ...
            {linkModal.open && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100]">
                    <div className="bg-white p-6 rounded-lg shadow-xl w-96 animate-popIn">
                        <h3 className="text-lg font-bold mb-4 font-serif text-[#176f6f]">Havola qo'shish</h3>
                        <input
                            type="text"
                            placeholder="URL manzili"
                            className="w-full p-2 border border-gray-200 rounded mb-3 text-sm focus:border-[#176f6f] outline-none"
                            value={linkModal.url}
                            onChange={e => setLinkModal({ ...linkModal, url: e.target.value })}
                        />
                        <input
                            type="text"
                            placeholder="Ko'rinadigan matn (ixtiyoriy)"
                            className="w-full p-2 border border-gray-200 rounded mb-4 text-sm focus:border-[#176f6f] outline-none"
                            value={linkModal.text}
                            onChange={e => setLinkModal({ ...linkModal, text: e.target.value })}
                        />
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setLinkModal({ ...linkModal, open: false })} className="px-4 py-2 text-gray-500 hover:bg-gray-100 rounded text-sm font-bold">BEKOR QILISH</button>
                            <button onClick={insertLink} className="px-4 py-2 bg-[#176f6f] text-white rounded text-sm font-bold shadow-lg hover:shadow-xl transition-all">QO'SHISH</button>
                        </div>
                    </div>
                </div>
            )}
